...
def readreshape(
    ...
    if i.ndim == 3:
        i = i[:, :, 0]
    # For microfluidics, all images are resized in 256x32
    if not crop:
        img = trans.resize(i, target_size, anti_aliasing=True, order=order)
    # For 2D, black space is added if img is smaller than target_size
    else:
        fill_shape = [
...
def getfnfromprototype(
    ...
    ----------
    prototype : str
        Filename prototype, written in c-style format. See utils.xpreader
        for more details
    fileorder : str
        Filenames ordering. See utils.xpreader for more details
