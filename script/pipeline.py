...
class Position:
    ...
    def preprocess(
        ...
        if self.drift_correction:
            self._msg("Drift correction started")
            ...
            self._msg("Drift correction complete")
    ...
    def features(
        ...
        self._msg("Found (%d fluorescent channels)" % (len(fluo_channels)),)
        ...
        if len(fluo_channels):
            ...
            if self.drift_correction:
                ...
                self._msg("Drift correction applied to fluorescent channels %d" % (f+1),)
        else:
            fluo_frames = None
    ...
    def save(
        ...
        if "legacy" in save_format:
            ...
            self._msg("Saved to legacy format\n%s" % filename + ".mat")
    ...
    def clear(self):
        ...
        for k in self.__dict__.keys():
            if k not in self._pickle_skip:
                setattr(self, k, None)
        self._msg("Cleared variables from memory")
